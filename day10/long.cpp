...
Pipe getStartPipeType(std::vector<std::string>& maze, std::pair<lld, lld> start, std::map<char, Pipe>& pipeTypes) {
    ...
    if (row>=0 && row<rows && (symbol = maze[row][col])!='.') {
        symbol = maze[row][col];
    ...
    // SOUTH
    row = start.first+1, col = start.second;
    if (row<rows && (symbol = maze[row][col])!='.') {
        symbol = maze[row][col];
        Pipe pipe = pipeTypes[symbol];
    ...
    if (col<cols && (symbol = maze[row][col])!='.') {
        symbol = maze[row][col];
        Pipe pipe = pipeTypes[symbol];
    ...
    // WEST
    row = start.first, col = start.second-1;
    if (col>=0 && col<cols && (symbol = maze[row][col])!='.') {
        symbol = maze[row][col];
        Pipe pipe = pipeTypes[symbol];
...
bool findLoop(std::vector<std::string>& maze, std::pair<lld, lld>& pos, std::vector<std::vector<lld>>& visited, Direction prevDir, std::map<char, Pipe>& pipeTypes) {
    if (visited[pos.first][pos.second] > 0) return true;
    ...
    return findLoop(maze, nextPos, visited, oppDir(nextDir), pipeTypes);
}

void connectGrounds(std::vector<std::string>& maze, std::vector<std::vector<lld>>& visited, std::size_t x, std::size_t y, lld label, std::map<char, Pipe>& pipeTypes) {
    std::size_t row, col, rows = maze.size(), cols = maze[0].length();
    ...
    // north
    row = x-1, col = y;
    if (row>=0 && row<rows && visited[row][col]==0) {
        connectGrounds(maze, visited, row, col, label, pipeTypes);
    }

    // south
    row = x+1, col = y;
    if (row<rows && visited[row][col]==0) {
        connectGrounds(maze, visited, row, col, label, pipeTypes);
    }

    // east
    row = x, col = y+1;
    if (col<cols && visited[row][col]==0) {
        connectGrounds(maze, visited, row, col, label, pipeTypes);
    }

    // west
    row = x, col = y-1;
    if (col>=0 && col<cols && visited[row][col]==0) {
        connectGrounds(maze, visited, row, col, label, pipeTypes);
    }
}

lld findEnclosed(std::vector<std::string>& maze, std::vector<std::vector<lld>>& visited, std::map<char, Pipe>& pipeTypes) {
    std::vector<std::vector<char>> ground(visited.size(), std::vector<char>(visited[0].size(), 'O'));
    lld totalInside = 0;
    
    for (std::size_t i=0; i<ground.size(); ++i) {
        bool outside = true;
        char prevChar = '$';
        for (std::size_t j=0; j<ground[0].size(); ++j) {
            // an intersection occured
            if (visited[i][j] == 1) {
                ground[i][j] = 'B';
                
                if (maze[i][j] == '|') outside = !outside;
                else if (maze[i][j]=='L') outside = !outside;
                else if (maze[i][j]=='J' && prevChar!='F') outside = !outside;
                else if (maze[i][j]=='F') outside = !outside;
                else if (maze[i][j]=='7' && prevChar!='L') outside = !outside;
                
                prevChar = (maze[i][j] != '-')? maze[i][j]: prevChar;
                if (outside) ground[i][j] = 'b';
            } else if (!outside) {
                ground[i][j] = 'I';
                totalInside++;
            }
    ...
    std::for_each(ground.begin(), ground.end(), [](auto &row) {
...
int main() {
    ...
    std::vector<std::vector<lld>> visited(maze.size(), std::vector<lld>(maze[0].length(), 0));

    /*std::for_each(visited.begin(), visited.end(), [](auto &dist) {
    ...
    findLoop(maze, pos, visited, oppDir(startPipe.from), pipeTypes);
    ...
    for (std::size_t i=0; i<maze.size(); ++i) {
        for (std::size_t j=0; j<maze[0].length(); ++j) {
            if (visited[i][j] == 0) {
                connectGrounds(maze, visited, i, j, label, pipeTypes);
                label++;
            }
        }
    }
    ...
    lld insidePoints = findEnclosed(maze, visited, pipeTypes);
    std::cout << "Points inside the loop: " << insidePoints << "\n";
