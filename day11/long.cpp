...
int main() {
    ...
    ulld expansion_factor = 1000000;
    std::cout << "Galaxies: " << galaxies.size() << "\n";
    std::for_each(galaxies.begin(), galaxies.end(), [&emptyRows, &emptyCols, expansion_factor](auto &p) {
        ulld x = p[0], y = p[1];
        std::for_each(emptyRows.begin(), emptyRows.end(), [&p, &x, expansion_factor](auto &r) {
            if (r<x) p[0]+=(expansion_factor-1);
        });

        std::for_each(emptyCols.begin(), emptyCols.end(), [&p, &y, expansion_factor](auto &c) {
            if (c<y) p[1]+=(expansion_factor-1);
        }); 
        // std::cout << "(" << p[0] << ", " << p[1] << ")\n"; 
    });
